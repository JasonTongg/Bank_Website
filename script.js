...
let balanceDate = document.querySelector(".content__left--balanceList--item--date");
...
//transfer
let transferContainer = document.querySelector(".content__right--transfer--container");
let transferUserName = document.querySelector(".content__right--transfer--transferTo--input");
let transferAmount = document.querySelector(".content__right--transfer--amount--input");
let transferButton = document.querySelector(".content__right--transfer--submit");

//request
let requestContainer = document.querySelector(".content__right--request--container");
let requestAmount = document.querySelector(".content__right--request--amount--input");
let requestButton = document.querySelector(".content__right--request--submit");

//close
let closeContainer = document.querySelector(".content__right--close--container");
let closeUserName = document.querySelector(".content__right--close--confirmUser--input");
let closePin = document.querySelector(".content__right--close--confirmPin--input");
...
//Alias
accs.forEach((item) => {
    let hasil = item.name.toLowerCase().split(" ").map((item2) => item2[0]).join("");
    item.alias = hasil;
})
...
navbarForm.addEventListener("submit", (e) => {
    ...
    accs.forEach((item, index) => {
        if (navUserName.value === item.alias && Number(navPin.value) === item.password) {
...
closeContainer.addEventListener("submit", (e) => {
    ...
    if (closeUserName.value !== currAccount.alias) {
        accs.some((item) => item.alias === closeUserName.value && item.password === Number(closePin.value)) ? accs.splice(accs.findIndex(item2 => item2.alias === closeUserName.value), 1) : null;
    }
...
transferContainer.addEventListener("submit", (e) => {
    e.preventDefault();
    if (transferUserName.value !== currAccount.alias && currAccount.totalBalance >= Number(transferAmount.value)) {
        if (accs.some((item) => item.alias === transferUserName.value)) {
            let index = accs[accs.findIndex(item2 => item2.alias === transferUserName.value)];
...
function displayList(acc) {
    ...
    acc.balanceHistory.forEach((item, index) => {
        ...
        <div
            class="content__left--balanceList--item--paragraphContainer ${item > 0 ? "deposit" : "withdraw"}"
        >
        ...
        <h2 class="content__left--balanceList--item--h2">${Intl.NumberFormat(`${acc.format}`,{style: "currency",currency: `${acc.MataUang}`}).format(item.toFixed(2))}
        </h2>
        </div>`
...
function displayInOut(list) {
    ...
    int = list.balanceHistory.filter(item => item > 0).map((item) => item * list.interest / 100).filter(item => item > 1).reduce((acc, curr) => acc + curr);

    balanceIn.innerHTML = `In <span>${Intl.NumberFormat(`${list.format}`,{style: "currency",currency: `${list.MataUang}`}).format(pos.toFixed(2))}</span>`;
    balanceOut.innerHTML = `Out <span>${Intl.NumberFormat(`${list.format}`,{style: "currency",currency: `${list.MataUang}`}).format(neg.toFixed(2) * -1)}</span>`;
    balanceInterest.innerHTML = `Interest <span>${Intl.NumberFormat(`${list.format}`,{style: "currency",currency: `${list.MataUang}`}).format(int.toFixed(2))}</span>`;
    ...
    autoLogOut = setInterval(() => {
        let min = milisec / 60;
        let sec = milisec % 60;
        logOutTime.textContent = `${Math.trunc(min)}`.padStart(2, 0) + `:` + `${sec}`.padStart(2, 0);
